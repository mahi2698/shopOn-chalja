    ...
    stages {
        ...
        stage('JAVA RUN'){
            steps{
                ...
                sh 'Maven_Directory=/usr/share/maven/repository && find $Maven_Directory -name '*.jar' | xargs -i cp -p '{}' $WORKSPACE/target/test-classes/ && ProjectPath=$WORKSPACE && classpath=$WORKSPACE/target/test-classes;$WORKSPACE/target/test-classes/* && java com.ShopOn.TestRunner.TestRunner'
            }
        }
    }
}
